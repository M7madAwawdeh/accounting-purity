...
<div class="container-fluid">
    <h1>{{ __('main.edit_expense_voucher') }}</h1>

    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('expense-vouchers.update', $expenseVoucher->id) }}" method="POST" id="transactionForm">
                @csrf
                @method('PUT')
                <input type="hidden" name="accountable_type" id="accountable_type" value="{{ old('accountable_type', $expenseVoucher->accountable_type) }}">
                <input type="hidden" name="accountable_id" id="accountable_id" value="{{ old('accountable_id', $expenseVoucher->accountable_id) }}">

                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="recipient">{{ __('main.recipient') }}</label>
                            <input type="text" name="recipient" class="form-control @error('recipient') is-invalid @enderror" value="{{ old('recipient', $expenseVoucher->recipient) }}" required>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $expenseVoucher->date) }}" required>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="amount" step="0.01" class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $expenseVoucher->amount) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <select name="currency" id="currency" class="form-control @error('currency') is-invalid @enderror" required>
                                ...
                                @foreach($currencies as $currency)
                                    <option value="{{ $currency->code }}" {{ old('currency', $expenseVoucher->currency) == $currency->code ? 'selected' : '' }}>{{ $currency->name }} ({{ $currency->symbol }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="project_id" id="project_id" class="form-control @error('project_id') is-invalid @enderror">
                                ...
                                @foreach($projects as $project)
                                    <option value="{{ $project->id }}" {{ old('project_id', $expenseVoucher->project_id) == $project->id ? 'selected' : '' }}>{{ $project->name }}</option>
                                @endforeach
                            </select>
                ...
                <div class="row" id="association_fee_row" style="display: none;">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" name="association_fee" step="0.01" class="form-control @error('association_fee') is-invalid @enderror" value="{{ old('association_fee', $expenseVoucher->association_fee) }}">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select name="payment_method" id="payment_method" class="form-control @error('payment_method') is-invalid @enderror" required>
                                ...
                                <option value="cash" {{ old('payment_method', $expenseVoucher->payment_method) == 'cash' ? 'selected' : '' }}>{{ __('main.cash') }}</option>
                                <option value="bank_transfer" {{ old('payment_method', $expenseVoucher->payment_method) == 'bank_transfer' ? 'selected' : '' }}>{{ __('main.bank_transfer') }}</option>
                                <option value="cheque" {{ old('payment_method', $expenseVoucher->payment_method) == 'cheque' ? 'selected' : '' }}>{{ __('main.cheque') }}</option>
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="number" class="form-control @error('number') is-invalid @enderror" value="{{ old('number', $expenseVoucher->cheque->number ?? '') }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="cheque_account_number" id="cheque_account_number" class="form-control @error('cheque_account_number') is-invalid @enderror" value="{{ old('cheque_account_number', $expenseVoucher->cheque->account_number ?? '') }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" name="cheque_due_date" class="form-control @error('cheque_due_date') is-invalid @enderror" value="{{ old('cheque_due_date', $expenseVoucher->cheque->due_date ?? '') }}">
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror">{{ old('description', $expenseVoucher->description) }}</textarea>
                    @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <button type="submit" class="btn btn-primary">{{ __('main.update') }}</button>
                <a href="{{ route('expense-vouchers.index') }}" class="btn btn-secondary">{{ __('main.cancel') }}</a>
            </form>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const selectedAccountable = {
        type: '{{ old('accountable_type', $expenseVoucher->accountable_type) }}',
        id: '{{ old('accountable_id', $expenseVoucher->accountable_id) }}'
    };
